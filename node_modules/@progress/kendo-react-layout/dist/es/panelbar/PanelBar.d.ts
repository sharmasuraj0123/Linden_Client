/// <reference types="react" />
import * as React from 'react';
import * as PropTypes from 'prop-types';
import { PanelBarItemProps } from './PanelBarItem';
import { PanelBarInterface, PanelBarItemClickEventArguments } from './PanelBarInterface';
import { NavigationAction } from './services/navigationService';
/**
 * Represents the props of the PanelBar component for React.
 */
export interface PanelBarProps extends PanelBarInterface {
    /**
     * The child can be either a single PanelBarItem or a PanelBarItem array.
     */
    children?: React.ReactNode;
    /**
     * The class name that is set to the PanelBar component.
     */
    className?: string;
}
/**
 * @hidden
 */
export interface PanelBarState {
    focused?: string;
    wrapperFocused?: boolean;
    selected?: string;
    expanded: string[];
}
export default class PanelBar extends React.Component<PanelBarProps, PanelBarState> {
    /**
     * @hidden
     */
    static propTypes: {
        animation: PropTypes.Requireable<any>;
        children: (props: PanelBarProps, propName: any) => Error;
        dir: PropTypes.Requireable<any>;
        selected: PropTypes.Requireable<any>;
        expanded: PropTypes.Requireable<any>;
        focused: PropTypes.Requireable<any>;
        expandMode: PropTypes.Requireable<any>;
        className: PropTypes.Requireable<any>;
        onSelect: PropTypes.Requireable<any>;
    };
    /**
     * @hidden
     */
    static defaultProps: {
        expandMode: string;
        animation: boolean;
    };
    private activeDescendant?;
    private _element;
    private _selectionService;
    private _navigationService;
    constructor(props: PanelBarProps);
    /**
     * @hidden
     */
    componentWillReceiveProps(newProps: PanelBarProps): void;
    /**
     * @hidden
     */
    handleSelect: (event: PanelBarItemClickEventArguments) => void;
    /**
     * @hidden
     */
    onSelect: (child: React.ReactElement<PanelBarItemProps>, expanded: string[]) => void;
    /**
     * @hidden
     */
    onFocus: (child: React.ReactElement<PanelBarItemProps>) => void;
    /**
     * @hidden
     */
    onNavigate: (action: NavigationAction) => void;
    /**
     * @hidden
     */
    render(): JSX.Element;
    private handleWrapperFocus;
    private handleKeyDown;
}
